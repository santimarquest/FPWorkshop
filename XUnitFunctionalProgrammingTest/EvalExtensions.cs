...
namespace CsharpPoker
{
    public static class EvalExtensions
    {
        ...
        public static IEnumerable<KeyValuePair<(CardValue, CardSuit), int>> ToKindAndQuantitiesWithSuit(this IEnumerable<Card> cards)
        {
            var dict = new ConcurrentDictionary<(CardValue, CardSuit), int>();
            foreach (var card in cards)
            {
                dict.AddOrUpdate((card.Value, card.Suit), 1, (cardValue, quantity) => ++quantity);
            }
            return dict;
        }
