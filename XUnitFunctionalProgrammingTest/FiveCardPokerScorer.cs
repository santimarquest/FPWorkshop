...
namespace CsharpPoker
{
    public class FiveCardPokerScorer
    {
        ...
        private static bool HasCardGroups(IEnumerable<Card> cards, int numCardsInGroup, int numGroups) => cards.ToKindAndQuantities().Count(c => c.Value == numCardsInGroup) == numGroups;
        ...
        private static List<(Func<IEnumerable<Card>, bool> eval, HandRank rank)> Rankings() =>
           new List<(Func<IEnumerable<Card>, bool> eval, HandRank rank)>
           {
                       ...
                       (cards => HasCardGroups(cards, 2, 2), HandRank.TwoPair),
